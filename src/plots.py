...
A fifth file is read and is **not** in that list, deliberately: ``reports/figures/layout.json``
carries no measurement, only where a human dragged a legend or a label to (see
:mod:`src.figure_layout`). It is committed for the same reason the figures are -- so that
``./run.sh`` reproduces the tuned layout rather than a fresh clone quietly reverting to whatever
``loc=`` this module happens to code -- and an absent or empty one renders exactly these defaults.
Placement is the only thing it may set; styling stays here, where it is documented.
...
from .figure_layout import (  # noqa: E402
    LAYOUT_JSON,
    Layout,
    interactive_backend,
    load_layout,
...
#: This is the *committed* format and the one the report embeds -- `./run.sh` writes exactly these
#: seven files and nothing else, so the byte-identity claim is about PNGs.
FIGURE_DPI: int = 200
FIGURE_FORMAT: str = "png"

#: Vector formats `--vector` additionally writes, into `EDITABLE_SUBDIR`. Opt-in and git-ignored:
#: they are an editing convenience (open in Inkscape/Illustrator, move anything, export), not a
#: deliverable, and committing them would put a second copy of every figure under version control
#: that nothing reads. Prefer `--tune` when the change is a position -- that one round-trips back
#: into the pipeline; reach for these when the change is something no override can express.
VECTOR_FORMATS: tuple[str, ...] = ("svg", "pdf")

#: Where `--vector` writes. A subdirectory rather than alongside the PNGs so `reports/figures/`
#: stays exactly the eight committed files (seven figures + README.md) plus layout.json.
EDITABLE_SUBDIR: str = "editable"
...
def apply_style() -> None:
    ...
    plt.rcParams.update({
        ...
        # SVG only, and both settings serve `--vector`. `fonttype: none` keeps labels as real text
        # rather than outlines, which is the difference between an SVG you can retype a label in and
        # one you can only push shapes around in. `hashsalt` fixes the element ids matplotlib would
        # otherwise derive from object addresses, so re-exporting an unchanged figure is a no-op
        # diff instead of a whole-file churn.
        "svg.fonttype": "none",
        "svg.hashsalt": "ids-crossera",
    })
...
@dataclass
class RenderSession:
    """What the current ``python -m src.plots`` invocation wants done with each finished figure.

    A module-level switch rather than a parameter on all seven figure functions, because it changes
    nothing about *what* they draw -- only what :func:`save_figure` does with the result. Three modes
    share one code path, so a tuned or vector-exported figure is by construction the same figure:

    * default -- write the committed PNG, close;
    * ``--vector`` -- also write ``out/editable/<name>.{svg,pdf}``, close;
    * ``--tune`` -- write nothing and keep the figure open, so the caller can hand it to
      :func:`figure_layout.tune`.
    """

    formats: tuple[str, ...] = (FIGURE_FORMAT,)
    keep_open: bool = False
    #: Whether to rewrite ``reports/figures/README.md``. False for a partial render: the sidecar is
    #: committed and holds all seven captions, and rebuilding it from a subset would truncate it to
    #: the figures that happened to be named.
    write_index: bool = True
    #: ``(name, figure, layout)`` for every figure drawn this session; only populated when
    #: ``keep_open``, since otherwise the figures are closed and the list would hold dead objects.
    held: list[tuple[str, Any, Any]] = field(default_factory=list)


#: The active session. Reassigned by :func:`main` from its parsed arguments and by the tuner; the
#: default value is what an ordinary ``import src.plots`` and every ``./run.sh`` run see.
_SESSION: RenderSession = RenderSession()


def session() -> RenderSession:
    """The active :class:`RenderSession`."""
    return _SESSION


def set_session(new: RenderSession) -> RenderSession:
    """Install ``new`` as the active session and return the one it replaced."""
    global _SESSION
    previous, _SESSION = _SESSION, new
    return previous


#: Metadata keys whose default value is a timestamp, per format. Blanked on every write: a figure
#: that embeds the moment it was rendered can never be byte-identical across runs, which is the one
#: property `./run.sh` is checked against.
_UNDATED: dict[str, dict[str, None]] = {
    "svg": {"Date": None},
...
def save_figure(
    ...
    ``layout`` is the figure's :class:`figure_layout.Layout`, passed so ``--tune`` can find the
    movable artists it recorded. Optional: a figure with nothing overridable (the pipeline diagram)
    has none.

    Closes ``fig`` on the way out -- a Phase 9 run draws several figures and matplotlib keeps every
    unclosed one alive -- unless the active :class:`RenderSession` is holding figures open to tune.
    """
    out = Path(out)
    out.mkdir(parents=True, exist_ok=True)
    active = session()
    path = out / f"{name}.{FIGURE_FORMAT}"
    for fmt in active.formats:
        target = path if fmt == FIGURE_FORMAT else out / EDITABLE_SUBDIR / f"{name}.{fmt}"
        target.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(
            target, dpi=FIGURE_DPI, bbox_inches="tight", metadata=_UNDATED.get(fmt),
        )
    if active.keep_open:
        active.held.append((name, fig, layout))
    else:
        plt.close(fig)
...
def _write_figure_index(out: Any = FIGURES) -> Path:
    ...
    lines = [
        ...
        f"is written to `{LAYOUT_JSON.name}` (committed, in this directory) and every later render —",
...
def plot_indist_vs_crossera(metrics: Any, out: Any = FIGURES) -> Path:
    ...
    layout = Layout.for_figure("drift_indist_vs_crossera")
    ...
    # apart on x and their labels would otherwise collide -- and each sits *beside* its own line
    # rather than centred on it, knocked out of the grid behind: centred, the dashed rule struck
    # through a digit of the very number the reader is meant to read off it.
    # The stagger is wider than the labels are tall because the lower of the two sits at x ~ 0.88,
    # and the Δ column header begins at 1.045: a tighter offset put its knockout box over the Δ.
    for metric_key, colour, label_y, side in (
        ("in_f1", REGIME_COLOURS["in_distribution"], header_y - 0.32, -1),
        ("cross_f1", REGIME_COLOURS["cross_era"], header_y + 0.42, +1),
    ):
        value = float(dummy[metric_key])
        axes[1].axvline(value, color=colour, linewidth=1.0, linestyle=(0, (3, 2)), zorder=4)
        layout.text(
            axes[1], f"text.dummy_f1.{metric_key}",
            value + side * 0.014, label_y, f"{value:.4f}",
            ha="left" if side > 0 else "right", va="center", fontsize=6.4, color=colour, zorder=5,
    ...
    return save_figure(fig, "drift_indist_vs_crossera", caption, sources, out=out, layout=layout)
...
def plot_recovery_curve(recovery: Any, out: Any = FIGURES) -> Path:
    ...
    **One figure, two panels** -- (a) ROC-AUC, (b) F1 -- rather than two files: both panels carry
    ...
    layout = Layout.for_figure("recovery_curve")
    ...
    return save_figure(fig, "recovery_curve", caption, sources, out=out, layout=layout)
...
def plot_roc_curves(results: Any, out: Any = FIGURES) -> Path:
    ...
    **Two panels, one per regime**, on identical square axes so the pair can be compared by eye: (a)
    ...
    # Taller than the other two-panel figures because each panel carries its own six-entry legend
    # *below* it (see the legend note further down) on top of a square plotting area.
    layout = Layout.for_figure("roc_curves")
    fig, axes = plt.subplots(1, 2, figsize=(PAGE_WIDTH_IN, 5.25))
    fig.subplots_adjust(left=0.085, right=0.995, top=0.825, bottom=0.310, wspace=0.20)
    ...
    for column, regime in enumerate(REGIME_ORDER):
        ...
        ax.plot([0.0, 1.0], [0.0, 1.0], color=INK_MUTED, linewidth=1.0, linestyle=(0, (3, 2)),
                zorder=2)
        # Labelled once, on the panel where it has occupants: in (a) the same triangle is empty, so
        # a second copy would annotate nothing.
        if regime == "cross_era":
            layout.text(
                ax, "text.chance_note",
        ...
        ax.grid(zorder=1)
        ax.set_axisbelow(True)
        # Below the panel, not inside it. This figure is the one place where the legend has to be
        # per-panel rather than shared -- each entry carries that panel's own ROC-AUC, and those are
        # the numbers the figure exists to show -- and the earlier in-axes placement cost real
        # legibility for it: six entries at this width filled (a)'s lower-right triangle and, in (b),
        # sat on top of the curve rise near fpr = 1. Under the axes it occludes nothing, and the two
        # legends still read as a pair because they are identically positioned.
        # Ordered by MODEL_ORDER (descending in-distribution AUC), so in (b) the legend deliberately
        # does NOT re-rank: a reader can follow one model across both panels.
        layout.legend(
            ax, f"legend.{regime}", handles, labels,
            loc="upper left", bbox_to_anchor=(-0.005, -0.135), ncol=1,
            fontsize=6.4, handlelength=2.4, borderpad=0.4, labelspacing=0.34,
    ...
    return save_figure(fig, "roc_curves", caption, sources, out=out, layout=layout)
...
def plot_per_family_f1(results: Any, out: Any = FIGURES) -> Path:
    ...
    **ROC-AUC leads** and F1 follows with its majority-class lines drawn on. The per-family F1
    ...
    layout = Layout.for_figure("per_family_crossera")
        ...
        for column, (metric, xlabel) in enumerate(
            ...
            if metric == "roc_auc":
                ...
                ax.axvline(chance, color=INK_SECONDARY, linewidth=1.0, linestyle=(0, (3, 2)),
                           zorder=4)
                layout.text(ax, f"text.chance.{family}",
                            chance - 0.03, header_y, f"chance {chance:.4f}", ha="right",
                            va="center", fontsize=6.0, color=INK_SECONDARY)
            else:
                # Beside each line rather than centred on it, and knocked out of the grid behind --
                # same reason as Figure 1's panel (b): these subsets put the two majority-class F1s
                # as close as 0.03 and 0.44, and a label centred on its own dashed rule is a label
                # with a digit struck through.
                for key, colour, label_y, preferred in (
                    ("in_f1", REGIME_COLOURS["in_distribution"], header_y - 0.32, -1),
                    ("cross_f1", REGIME_COLOURS["cross_era"], header_y + 0.30, +1),
                ):
                    value = float(dummy[key])
                    # A left-hand label flips right when its own line sits too close to x = 0 to fit
                    # one: `backdoor`'s in-distribution floor is 0.0306, and a right-aligned label
                    # there would hang off the axes into the model names. The two are staggered on
                    # y, so both sitting on the right is not a collision.
                    side = preferred if value > 0.14 else +1
                    ax.axvline(value, color=colour, linewidth=1.0, linestyle=(0, (3, 2)), zorder=4)
                    layout.text(
                        ax, f"text.dummy_f1.{family}.{key}",
                        value + side * 0.018, label_y, f"{value:.4f}",
                        ha="left" if side > 0 else "right", va="center", fontsize=6.0,
    ...
    return save_figure(fig, "per_family_crossera", caption, sources, out=out, layout=layout)
...
def plot_per_family_recovery(results: Any, out: Any = FIGURES) -> Path:
    ...
    **A different population from Figure 5, and deliberately so.** This is a *within*-era
    ...
    layout = Layout.for_figure("per_family_recovery")
    ...
    for index, family in enumerate(families):
        ...
        layout.text(ax, f"text.floor.{family}",
                    -0.28, floor - 0.045, f"floor {floor:.3f}", ha="left", va="top", fontsize=5.9,
    ...
    return save_figure(fig, "per_family_recovery", caption, sources, out=out, layout=layout)
...
def builders(metrics: pd.DataFrame) -> dict[str, Any]:
    """The seven figures, keyed by output stem, in generation order.

    A registry rather than a straight-line sequence of calls so ``--tune`` and ``--vector`` can be
    pointed at a single figure by the name of the file it writes, and so the order the report's
    figure numbering depends on is stated once. Every value takes only ``out``; what each closes
    over is the point of the comments below.
    """
    return {
        "drift_indist_vs_crossera": lambda out: plot_indist_vs_crossera(metrics, out=out),
        "recovery_curve": lambda out: plot_recovery_curve(metrics, out=out),
        ...
        "confusion_matrices": lambda out: plot_confusion_matrices(None, out=out),
        ...
        "roc_curves": lambda out: plot_roc_curves(None, out=out),
        # The next two read reports/per_family_metrics.csv, again via `None`. They are TWO figures
        ...
        "per_family_crossera": lambda out: plot_per_family_f1(None, out=out),
        "per_family_recovery": lambda out: plot_per_family_recovery(None, out=out),
        ...
        "pipeline": lambda out: plot_pipeline(metrics, out=out),
    }


def _resolve_names(requested: Sequence[str], available: Iterable[str]) -> list[str]:
    """Validate figure names against the registry, keeping the registry's order.

    Rejects an unknown name rather than skipping it: a typo'd ``--tune`` that silently tuned nothing
    would look identical to a tuning session the user forgot to save.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    This is the entry point ``run.sh``'s Phase 9 line calls, and with no arguments that is all it
    does: render the seven committed PNGs plus the caption sidecar. The two optional modes exist so
    a *human* can improve a layout without breaking the reproducibility claim -- see
    :mod:`src.figure_layout`:

    * ``--tune [name ...]`` opens the figures in a window with their legends and dashed-line labels
      draggable, and saves what you dragged to ``reports/figures/layout.json``, which every later
      render reads. This is the mode to reach for first: the result is still pipeline output.
    * ``--vector`` additionally writes ``reports/figures/editable/<name>.{svg,pdf}`` for edits no
      override can express. Those are git-ignored scratch: whatever you make of them, the committed
      figure is still the PNG this command renders.
    """
    ...
    parser.add_argument(
        ...
        help="render only these figures, by output stem (default: all seven)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "open the figures in an interactive window with draggable legends and dashed-line "
            f"labels; press s to save their positions to {LAYOUT_JSON.name}, which every later "
            "render reads. Writes no PNG."
        ),
    )
    parser.add_argument(
        "--vector", action="store_true",
        help=(
            f"also write {'/'.join(VECTOR_FORMATS)} into {EDITABLE_SUBDIR}/ for editing in "
            "Inkscape or Illustrator (git-ignored; the committed figure stays the PNG)"
        ),
    )
    args = parser.parse_args(argv)

    registry_names = list(builders(pd.DataFrame(columns=list(METRICS_HEADER))))
    if args.list:
    ...
    out = Path(args.out)
    if args.tune:
        # Before any figure exists: `Agg` is chosen at import time for `run.sh`'s benefit, and a
        # backend switch after a canvas has been created does not take.
        print(f"interactive backend: {interactive_backend()}")

    metrics = load_metrics()
    print(f"read {len(metrics):,} rows across {metrics['run_id'].nunique()} run_ids "
          f"from {METRICS_CSV}")
    committed = load_layout()
    if committed:
        tuned = sum(len(elements) for elements in committed.values())
        print(f"read {tuned} tuned position(s) across {len(committed)} figure(s) from {LAYOUT_JSON}")

    registry = builders(metrics)
    names = _resolve_names(args.figures, registry)
    formats = (FIGURE_FORMAT, *VECTOR_FORMATS) if args.vector else (FIGURE_FORMAT,)
    complete = len(names) == len(registry)
    previous = set_session(RenderSession(
        formats=() if args.tune else formats,
        keep_open=args.tune,
        write_index=complete and not args.tune,
    ))
    try:
        for name in names:
            path = registry[name](out)
            if not args.tune:
                print(f"  wrote {path}")
                for fmt in VECTOR_FORMATS if args.vector else ():
                    print(f"  wrote {out / EDITABLE_SUBDIR / f'{name}.{fmt}'}")
        if args.tune:
            return _tune_held(session().held)
        if complete:
            print(f"  wrote {out / FIGURE_INDEX_NAME}  ({len(_FIGURE_INDEX)} captions)")
        else:
            print(f"  {out / FIGURE_INDEX_NAME} left as-is (partial render of "
                  f"{len(names)}/{len(registry)} figures)")
    finally:
        set_session(previous)
    return 0


def _tune_held(held: Sequence[tuple[str, Any, Any]]) -> int:
    """Make every figure held open by ``--tune`` draggable, then hand over to the GUI event loop.

    One :func:`~matplotlib.pyplot.show` for all of them rather than one per figure, so tuning two
    figures that must agree with each other (Figures 5 and 6, say) can be done side by side.
    """
    if not held:  # pragma: no cover - `_resolve_names` rejects an empty selection first
        print("nothing to tune")
        return 1
    handles = [tune(fig, layout, show=False) for _, fig, layout in held]
    print(f"\n{len(held)} window(s) open — close them all to exit")
    plt.show()
    del handles  # kept alive only for the duration of the event loop
    return 0
